    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="relative w-36 h-36">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
          ...
          <circle cx="60" cy="60" r="52" fill="none" stroke={seconds >= targetSeconds ? "#22c55e" : "#4ade80"} strokeWidth="8" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 52}`} strokeDashoffset={`${2 * Math.PI * 52 * (1 - progress / 100)}`} className="transition-all duration-1000" />
        </svg>
      ...
      {!isRunning ? (
        <button onClick={startTimer} className="px-8 py-3 bg-brand-500 text-white rounded-full font-semibold hover:bg-brand-600 transition-colors text-lg">
          {seconds > 0 ? "Resume" : "Start"}
        </button>
      ) : (
        <button onClick={stopAndRecord} className="px-8 py-3 bg-red-500 text-white rounded-full font-semibold hover:bg-red-600 transition-colors text-lg">
